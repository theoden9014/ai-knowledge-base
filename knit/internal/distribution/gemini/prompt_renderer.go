...
func (promptRenderer) Render(e *source.Entry, _ *source.Pack) ([]source.Artifact, error) {
	...
	for k, val := range e.FrontmatterFor(Target) {
		...
		if !isTOMLEncodable(val) {
			return nil, fmt.Errorf("%w: prompt %q: unsupported value type %T at key %q", ErrUnsupportedFrontmatterValue, e.ID, val, k)
		}
		v[k] = val
	}
	...
	if err := enc.Encode(v); err != nil {
		return nil, fmt.Errorf("%w: prompt %q: %v", ErrUnsupportedFrontmatterValue, e.ID, err)
	}
